...
/*
  ...
  UI Conventions
  - Force values are converted to kN internally.
  - Moment values are converted to kN·m internally.
  - Positive Fx = right
  - Negative Fx = left
  - Positive Fy input = downward
  - Negative Fy input = upward
  - Positive Mz = CCW
  - SVG screen Y is downward.
...
function RenderSupportSVG({ cx, cy, type, dir = 'horizontal' }) {
  ...
  if (type === 'pin') {
    return vertical ? (
      <g>
        <polygon
          points={`${cx - 5},${cy} ${cx - 20},${cy - 10} ${cx - 20},${cy + 10}`}
          fill={color}
        />
        <line
          x1={cx - 20}
      ...
      <g>
        <polygon
          points={`${cx},${cy + 5} ${cx - 10},${cy + 20} ${cx + 10},${cy + 20}`}
          fill={color}
        />
        <line
          x1={cx - 15}
...
function SpaceTelescopeSVG({ x, y, scale = 1 }) {
  return (
    <g transform={`translate(${x} ${y}) scale(${scale})`}>
      <polygon
        points="-24,-8 18,-12 28,2 -16,10"
        fill="#d9d9d9"
        stroke="#ffffff"
        strokeWidth="2"
      />
      <polygon
        points="18,-12 34,-7 38,2 28,2"
        fill="#E7B94F"
        stroke="#ffffff"
        strokeWidth="1.5"
      />
      <rect
        ...
        strokeWidth="1.5"
      />
      <line x1="-39" y1="-10" x2="-31" y2="-10" stroke="#8cc9ff" strokeWidth="1" />
      <line x1="-39" y1="-4" x2="-31" y2="-4" stroke="#8cc9ff" strokeWidth="1" />
      <line x1="-39" y1="2" x2="-31" y2="2" stroke="#8cc9ff" strokeWidth="1" />
      <line x1="-39" y1="8" x2="-31" y2="8" stroke="#8cc9ff" strokeWidth="1" />
      <rect
        x="36"
        y="-10"
        width="14"
        ...
        strokeWidth="1.5"
      />
      <line x1="39" y1="-6" x2="47" y2="-6" stroke="#8cc9ff" strokeWidth="1" />
      <line x1="39" y1="0" x2="47" y2="0" stroke="#8cc9ff" strokeWidth="1" />
      <line x1="39" y1="6" x2="47" y2="6" stroke="#8cc9ff" strokeWidth="1" />
      <line x1="-2" y1="8" x2="-2" y2="26" stroke="#ffffff" strokeWidth="2" />
      <circle cx="-2" cy="30" r="4" fill="#cfcfcf" stroke="#ffffff" strokeWidth="1.5" />
      <text
        x="0"
        y="48"
        textAnchor="middle"
...
function App() {
  ...
  const getVectorComponents = useCallback(
    (v) => {
  ...
  }, [vectorLoads, vectorUnit, getVectorComponents])
  ...
  const updateBeamSupport = (id, field, value) => {
    saveBeamState()
    setBeamSupports((prev) =>
      prev.map((s) =>
        s.id === id ? { ...s, [field]: field === 'x' ? Number(value) : value } : s
      )
    )
    setChartData([])
  }
      ...
      prev.map((l) =>
        l.id === id
          ? {
              ...l,
              [field]:
                field === 'magnitude' || field === 'x' || field === 'start_x' || field === 'end_x'
                  ? Number(value)
                  : value,
            }
          : l
  ...
  const analyzeBeam = useCallback(async () => {
    setErrorMessage('')
    ...
    for (const l of beamLoads) {
      ...
      if (l.type === 'distributed') {
        if (
          Number(l.start_x) < 0 ||
          Number(l.end_x) > safeBeamLength ||
          Number(l.end_x) <= Number(l.start_x)
        ) {
    ...
    try {
      ...
      if (!response.data?.diagram_data?.x) {
        throw new Error('Invalid response from beam solver.')
      }
      ...
      console.error(err)
      setErrorMessage(
        err?.response?.data?.detail || err.message || 'Beam calculation failed.'
      )
    } finally {
      setIsAnalyzing(false)
    }
  }, [safeBeamLength, beamSupports, beamLoads, forceUnit])
  ...
  const handleTrussNodeClick = (e, node) => {
    ...
    if (selectedNodeId) {
      ...
      if (!duplicate && selectedNodeId !== node.id) {
        ...
        setElements((prev) => [
          ...prev,
          { id: Date.now(), n1: selectedNodeId, n2: node.id },
        ])
      }
    }
  ...
  const handleTrussCanvasClick = (e) => {
    ...
    if (selectedNodeId) {
      setElements((prev) => [
        ...prev,
        { id: Date.now() + 1, n1: selectedNodeId, n2: id },
      ])
    }
    ...
    setTrussSupports((prev) => {
      ...
      if (type === 'none' || type === 'free') {
        ...
        next[nodeId] = {
          ...(next[nodeId] || {}),
          type,
          direction: next[nodeId]?.direction || 'horizontal',
        }
      }
      return next
    })
    ...
    setTrussLoads((prev) => {
      ...
      if (value === '') {
        if (next[nodeId]) {
          ...
          if (!Object.keys(next[nodeId]).length) {
            delete next[nodeId]
          }
        }
      } else {
  ...
  const autoCleanMesh = (nds, els) => {
    ...
    els.forEach((el) => {
      ...
      for (let i = 0; i < path.length - 1; i++) {
        ...
        generated.push({ n1: Math.min(n1, n2), n2: Math.max(n1, n2) })
      }
    })
  ...
  const calculateTrussReactions = useCallback(() => {
    const supportRows = []
    ...
    Object.entries(trussSupports).forEach(([id, s]) => {
      ...
      // Fixed: Converted to meters & inverted Y axis to align with statics formulation
      const x = (node.x / PIXELS_PER_GRID) * gridScale
      const y = -(node.y / PIXELS_PER_GRID) * gridScale

      if (s.type === 'pin') {
        supportRows.push({ nodeId: node.id, component: 'fx', ux: 1, uy: 0, x, y })
        supportRows.push({ nodeId: node.id, component: 'fy', ux: 0, uy: 1, x, y })
    ...
    const A = supportRows.map((r) => [r.ux, r.uy, r.x * r.uy - r.y * r.ux])
    ...
    if (!sol) {
      return {
        error: 'Support geometry is singular; no unique reaction solution exists.',
      }
    }
  ...
  }, [trussSupports, nodes, gridScale, trussLoads, trussUnit])

  const runTrussAnalysis = useCallback(async () => {
    setErrorMessage('')
    ...
    try {
      ...
      if (local.error) {
        ...
        steps.push('1) Global equilibrium solver: +Fx right, +Fy up internally.')
        steps.push('2) UI load convention: +Fy is downward, −Fy is upward.')
        steps.push(`3) ΣFx residual = ${formatNumber(fx, 5)} ${trussUnit}`)
        steps.push(`4) ΣFy residual = ${formatNumber(fy, 5)} ${trussUnit}`)
        steps.push(`5) ΣM residual = ${formatNumber(mz, 5)} kN·m`)
        steps.push('6) Reactions were solved from the three global equilibrium equations.')
      }
      ...
      if (!response.data) {
        throw new Error('No data from truss solver.')
      }
      ...
      console.error(err)
      setErrorMessage(
        err?.response?.data?.detail || err.message || 'Truss analysis failed.'
      )
    } finally {
      setIsAnalyzing(false)
    }
  }, [nodes, elements, calculateTrussReactions, gridScale, trussSupports, trussLoads, trussUnit])
  ...
  const handleFrameNodeClick = (e, node) => {
    ...
    if (fSelectedNodeId) {
      const duplicate = fElements.some(
        (el) =>
      ...
      if (!duplicate) {
        ...
        setFElements((prev) => [
          ...prev,
          { id: Date.now(), n1: fSelectedNodeId, n2: node.id },
        ])
      }
    }
  ...
  const handleFrameCanvasClick = (e) => {
    ...
    if (fSelectedNodeId) {
      setFElements((prev) => [
        ...prev,
        { id: Date.now() + 1, n1: fSelectedNodeId, n2: id },
      ])
    }
    ...
    setFDistLoads((prev) => {
      ...
      if (value === '') {
        if (next[elId]) {
          ...
          if (!Object.keys(next[elId]).length) {
            delete next[elId]
          }
        }
      } else {
  ...
  const framePointLocation = useCallback(
    (el, p) => {
  ...
  const calculateFrameReactions = useCallback(() => {
    const rows = []
    ...
    Object.entries(fSupports).forEach(([id, s]) => {
      ...
      if (s.type === 'pin') {
        rows.push({ nodeId: node.id, component: 'fx', ux: 1, uy: 0, mz: 0, x, y })
        rows.push({ nodeId: node.id, component: 'fy', ux: 0, uy: 1, mz: 0, x, y })
      } else if (s.type === 'roller') {
        const vertical = (s.direction || 'horizontal') === 'horizontal'
        rows.push({
          nodeId: node.id,
        ...
        rows.push({ nodeId: node.id, component: 'fx', ux: 1, uy: 0, mz: 0, x, y })
        rows.push({ nodeId: node.id, component: 'fy', ux: 0, uy: 1, mz: 0, x, y })
        rows.push({ nodeId: node.id, component: 'mz', ux: 0, uy: 0, mz: 1, x, y })
      }
    })
    ...
    if (rows.length !== 3) {
      return {
        error:
          'This statics reaction solver requires exactly 3 independent reaction components. For a fixed support, use a single fixed support with no other reactions.',
      }
    }
    ...
    const A = rows.map((r) => [r.ux, r.uy, r.x * r.uy - r.y * r.ux + r.mz])
    ...
    rows.forEach((r, i) => {
      if (!rxns[r.nodeId]) {
        rxns[r.nodeId] = { fx: 0, fy: 0, mz: 0 }
      }
      rxns[r.nodeId][r.component] =
        r.component === 'mz'
  ...
  }, [fSupports, fNodes, fGridScale, fLoads, fForceUnit, fDistLoads, fElements, fPointLoadsOnElement, framePointLocation])

  const runFrameStaticsAnalysis = useCallback(() => {
    setErrorMessage('')
    setIsAnalyzing(true)
    ...
    try {
      ...
      const steps = [
        ...
        'Internal equilibrium convention: +Fx right, +Fy up, +Mz CCW.',
        'UI force convention: +Fx right, +Fy downward, −Fy upward.',
  ...
  }, [calculateFrameReactions, fForceUnit])
  ...
  }, [currentView, activeTab, analyzeParticle, analyzeVectors, analyzeBeam, runTrussAnalysis, runFrameStaticsAnalysis])
  ...
  const maxAbsoluteShear = chartData.length
    ? Math.max(...chartData.map((d) => Math.abs(d.shear || 0)))
    : 0

  const maxAbsoluteMoment = chartData.length
    ? Math.max(...chartData.map((d) => Math.abs(d.moment || 0)))
    : 0
  ...
  const renderDistributedLoadArrows = (
    x1,
    y1,
    x2,
    y2,
    ...
    for (let i = 0; i <= count; i++) {
      ...
      if (hasY) {
        const positiveUp = positiveVertical === 'up'
        const directionY = positiveUp ? -1 : 1
        const bx = px - nx * offset
        const by = py - ny * offset
        const tipX = bx
        const tipY = by
        const tailX = bx
        const tailY = by - 34 * directionY
      ...
      if (hasX) {
        const bx = px + nx * offset
        const by = py + ny * offset
        const directionX = Number(wx) > 0 ? 1 : -1
        const tailX = bx - 34 * directionX
        const tailY = by
    ...
    let yTailLine = null
    if (hasY) {
      const startBx = x1 - nx * offset
      const startBy = y1 - ny * offset
      const endBx = x2 - nx * offset
      const endBy = y2 - ny * offset
      const positiveUp = positiveVertical === 'up'
      const directionY = positiveUp ? -1 : 1
      ...
      yTailLine = (
        <line
          x1={startBx}
          y1={startBy - 34 * directionY}
          x2={endBx}
          y2={endBy - 34 * directionY}
          stroke={theme.supportOrange}
          strokeWidth="2.2"
        />
      )
    ...
    if (hasX) {
      ...
      xTailLine = (
        <line
          x1={startBx - 34 * directionX}
          y1={startBy}
          x2={endBx - 34 * directionX}
          y2={endBy}
          stroke={theme.supportOrange}
          strokeWidth="2.2"
        />
      ...
      {showFormulaModal && (
        ...
        >
          ...
          >
            ...
            <h3>Important</h3>
            <p>
              All calculations use a consistent sign convention internally and SI-derived force units.
            </p>
          </div>
        </div>
      )}
        ...
        >
          ...
          >
            {[
              [
                'Engineering Statics',
                'Particle equilibrium, force vectors, beams, trusses and frame reactions.',
                'statics',
              ],
              [
                'Mechanics of Materials',
                'Stress, strain, bending and torsion analysis.',
                null,
              ],
              [
                'Theory of Structures',
                'Indeterminate structures, influence lines and energy methods.',
                null,
              ],
              [
                'Structural Analysis',
                'Matrix methods, stiffness method and finite-element models.',
                null,
              ],
            ].map(([title, desc, view]) => (
              <div
        ...
        <div style={{ width: '100%' }}>
          ...
          {activeTab === 'particle' && (
            <div className="report-document">
              ...
              <p style={{ color: '#888' }}>
                Two-cable equilibrium with an interactive joint position.
              </p>
              ...
              >
                <div style={{ background: '#151515', padding: 15, borderRadius: 8 }}>
                  ...
                  <svg
                    ...
                    height="320"
                    viewBox="0 0 600 350"
                    onClick={handleParticleCanvasClick}
                  ...
                  >
                    ...
                    <circle cx="65" cy="100" r="6" fill={theme.supportOrange} />
                    <circle cx="535" cy="100" r="6" fill={theme.supportOrange} />
                    <line
                      x1="65"
                      y1="100"
                      x2={jointPos.x}
                ...
                <div style={{ background: '#151515', padding: 15, borderRadius: 8 }}>
                  ...
                  <svg width="100%" height="320" viewBox="0 0 600 350">
                    ...
                    <line
                      ...
                      markerEnd="url(#arrowPoint)"
                    />
                    <text x="185" y="110" fill={theme.accent}>
                      T₁
                    </text>
                    <text x="420" y="110" fill={theme.accent}>
                      T₂
                    </text>
                    <SpaceTelescopeSVG x={300} y={245} scale={0.75} />
                    <text x="515" y="170" fill="#aaa">
                      +x
                    </text>
                    <text x="310" y="45" fill="#aaa">
                      +y
                    </text>
                  </svg>
                </div>
                ...
                >
                  ...
                  <label>
                    ...
                    <input
                      ...
                      max="180"
                      value={pAngle1}
                      onChange={(e) => setPAngle1(e.target.value)}
                      style={{ ...inputStyle, display: 'block' }}
                    />
                  </label>
                  <label>
                    ...
                    <input
                      ...
                      max="180"
                      value={pAngle2}
                      onChange={(e) => setPAngle2(e.target.value)}
                      style={{ ...inputStyle, display: 'block' }}
                    />
                  </label>
                  <label>
                  ...
                  <button style={buttonStyle} onClick={analyzeParticle}>
              ...
              {particleResult && (
                ...
                >
                  ...
                  >
                    <b>
                      T₁ = {formatNumber(particleResult.T1)} {pUnit}
                    </b>
                    <b>
                      T₂ = {formatNumber(particleResult.T2)} {pUnit}
                    </b>
                  </div>
                </div>
              )}
                  ...
                  {vectorLoads.map((v, i) => {
                    ...
                    const vmax = vectorResult
                      ? Math.max(
                          1,
                          ...vectorLoads.map(
                            (x) => Math.abs(Number(x.magnitude)) || 0
                          ),
                          Math.abs(vectorResult.rMag)
                        )
                      : Math.max(
                          1,
                          ...vectorLoads.map(
                    ...
                    const sf = 210 / vmax
                    const x2 =
                      500 + Number(v.magnitude) * Math.cos(c.drawRad) * sf
                    const y2 =
                      300 - Number(v.magnitude) * Math.sin(c.drawRad) * sf
                  ...
                  {vectorResult?.rMag > 1e-8 && (
                    <line
                      ...
                      x2={
                        500 +
                        vectorResult.rMag *
                          Math.cos((vectorResult.rAng * Math.PI) / 180) *
                          (210 /
                            Math.max(
                              1,
                              ...vectorLoads.map(
                                (v) => Math.abs(Number(v.magnitude)) || 0
                              )
                            ))
                      }
                      y2={
                        300 -
                        vectorResult.rMag *
                          Math.sin((vectorResult.rAng * Math.PI) / 180) *
                          (210 /
                            Math.max(
                              1,
                              ...vectorLoads.map(
                  ...
                  >
                    ...
                    <label>
                      ...
                      <input
                        ...
                        value={v.magnitude}
                        onChange={(e) =>
                          updateVectorLoad(v.id, 'magnitude', e.target.value)
                        }
                    ...
                    <label>
                      ...
                      <input
                        ...
                        value={v.angle}
                        onChange={(e) =>
                          updateVectorLoad(v.id, 'angle', e.target.value)
                        }
                    ...
                    <select
                      value={v.quadrant}
                      onChange={(e) =>
                        updateVectorLoad(v.id, 'quadrant', Number(e.target.value))
                      }
                      style={inputStyle}
                    >
                    ...
                    <select
                      value={v.refAxis}
                      onChange={(e) =>
                        updateVectorLoad(v.id, 'refAxis', e.target.value)
                      }
                      style={inputStyle}
                    >
                      <option value="x">from X</option>
                      <option value="y">from Y</option>
                    </select>
                    <select
                      value={v.direction}
                      onChange={(e) =>
                        updateVectorLoad(v.id, 'direction', e.target.value)
                      }
                      style={inputStyle}
                    >
              ...
              {vectorResult && (
                ...
                >
                  ...
                  >
                    <thead>
                      <tr>
                        <th style={{ textAlign: 'left' }}>Force</th>
                        <th>Fx</th>
                        <th>Fy</th>
                      </tr>
                    </thead>
                    <tbody>
                      {vectorResult.steps.map((s) => (
                        <tr key={s.id}>
                          ...
                          <td style={{ textAlign: 'right' }}>
                            {formatNumber(s.fx)} {vectorUnit}
                          </td>
                          <td style={{ textAlign: 'right' }}>
                            {formatNumber(s.fy)} {vectorUnit}
                          </td>
                        </tr>
                      ))}
                      <tr style={{ borderTop: '2px solid #555' }}>
                        ...
                        <td style={{ textAlign: 'right' }}>
                          <b>
                            {formatNumber(vectorResult.sumFx)} {vectorUnit}
                          </b>
                        </td>
                        <td style={{ textAlign: 'right' }}>
                          <b>
                            {formatNumber(vectorResult.sumFy)} {vectorUnit}
                          </b>
                        </td>
                      </tr>
                    </tbody>
                  ...
                  <p>
                    <b>
                      |R| = {formatNumber(vectorResult.rMag)} {vectorUnit}
                    </b>
                  </p>
                  <p>
                    <b>
                      Direction = {formatNumber(vectorResult.rAng)}° (
                      {vectorResult.dirSymbol})
                    </b>
                  </p>
                </div>
              )}
          ...
          {activeTab === 'beam' && (
            <div className="report-document">
              <h1>Beam Analysis</h1>
              <p style={{ color: '#888' }}>
                Reactions, SFD and BMD are obtained from the configured beam solver.
              </p>
              ...
              >
                <button
                  style={buttonStyle}
                  onClick={() => loadBeamPreset('simply-supported')}
                >
                  Simply Supported
                </button>
                <button
                  style={buttonStyle}
                  onClick={() => loadBeamPreset('overhanging')}
                >
                  Overhanging
                </button>
                <button
                  style={buttonStyle}
                  onClick={() => loadBeamPreset('cantilever')}
                >
                  Cantilever
                </button>
                <button
                  style={buttonStyle}
                  onClick={handleUndoBeam}
                  disabled={!beamHistory.length}
                >
                  Undo
                </button>
              </div>
                    ...
                    {beamSupports.map((s) => (
                      ...
                      >
                        <select
                          value={s.type}
                          onChange={(e) =>
                            updateBeamSupport(s.id, 'type', e.target.value)
                          }
                          style={inputStyle}
                        >
                        ...
                        <input
                          ...
                          value={s.x}
                          onChange={(e) =>
                            updateBeamSupport(s.id, 'x', e.target.value)
                          }
                      ...
                      {l.type === 'point' && (
                        ...
                        >
                          ...
                          <input
                            ...
                            value={l.magnitude}
                            onChange={(e) =>
                              updateBeamLoad(l.id, 'magnitude', e.target.value)
                            }
                            style={{ ...inputStyle, width: 65 }}
                          />
                          <input
                            ...
                            value={l.x}
                            onChange={(e) =>
                              updateBeamLoad(l.id, 'x', e.target.value)
                            }
                            style={{ ...inputStyle, width: 65 }}
                          />
                          <button
                      ...
                      {l.type === 'moment' && (
                        ...
                        >
                          ...
                          <select
                            value={l.direction}
                            onChange={(e) =>
                              updateBeamLoad(l.id, 'direction', e.target.value)
                            }
                            style={inputStyle}
                          >
                      ...
                      {l.type === 'distributed' && (
                        ...
                        >
                          ...
                          <input
                            ...
                            value={l.start_x}
                            onChange={(e) =>
                              updateBeamLoad(l.id, 'start_x', e.target.value)
                            }
                            style={{ ...inputStyle, width: 60 }}
                          />
                          <input
                            ...
                            value={l.end_x}
                            onChange={(e) =>
                              updateBeamLoad(l.id, 'end_x', e.target.value)
                            }
                            style={{ ...inputStyle, width: 60 }}
                          />
                          <button
              ...
              <div style={{ textAlign: 'center', margin: 20 }}>
                <button
                  style={{ ...buttonStyle, padding: '13px 25px' }}
                  onClick={analyzeBeam}
                >
                  Analyze Beam
                </button>
              ...
              {beamReactions.length > 0 && (
                <div
                  style={{
                    padding: 15,
                    background: '#1A1A1A',
                    borderLeft: `5px solid ${theme.supportOrange}`,
                  }}
                >
                  ...
                  {beamReactions.map((r, i) => (
                    <span key={i} style={{ marginRight: 25 }}>
                      R @ x={r.support_x}:{' '}
                      <b>
                        {formatNumber(r.forceDisplay)} {forceUnit}
                      </b>
                    </span>
                  ))}
                </div>
                      ...
                      <ResponsiveContainer>
                        <AreaChart data={chartData}>
                          ...
                          <XAxis
                            dataKey="x"
                            type="number"
                            domain={[0, safeBeamLength]}
                            ticks={optimizedTicks}
                          />
                          ...
                          {chartShear.max && (
                            <ReferenceDot
                              x={chartShear.max.x}
                              y={chartShear.max.shear}
                              r={4}
                              label={`Max ${formatNumber(chartShear.max.shear)}`}
                            />
                          )}
                          {chartShear.min && (
                            <ReferenceDot
                              x={chartShear.min.x}
                              y={chartShear.min.shear}
                              r={4}
                              label={`Min ${formatNumber(chartShear.min.shear)}`}
                            />
                          )}
                        </AreaChart>
                      </ResponsiveContainer>
                      ...
                      <ResponsiveContainer>
                        <AreaChart data={chartData}>
                          ...
                          {chartMoment.max && (
                            <ReferenceDot
                              x={chartMoment.max.x}
                              y={chartMoment.max.moment}
                              r={4}
                              label={`Max ${formatNumber(chartMoment.max.moment)}`}
                            />
                          )}
                          {chartMoment.min && (
                            <ReferenceDot
                              x={chartMoment.min.x}
                              y={chartMoment.min.moment}
                              r={4}
                              label={`Min ${formatNumber(chartMoment.min.moment)}`}
                            />
                          )}
                        </AreaChart>
                      </ResponsiveContainer>
          ...
          {activeTab === 'truss' && (
            <div className="report-document">
              ...
              >
                <button style={buttonStyle} onClick={loadTrussPreset}>
                  Pratt Truss
                </button>
                <button
                  style={buttonStyle}
                  onClick={handleUndoTruss}
                  disabled={!trussHistory.length}
                >
                  Undo
                </button>
                <button style={buttonStyle} onClick={clearTrussCanvas}>
                  Clear
                </button>
                ...
                <label style={{ marginLeft: 'auto' }}>
                  ...
                  >
                    {Array.from({ length: 30 }, (_, i) => (i + 1) / 10).map((v) => (
                ...
                <label>
                  ...
                  <select
                    value={trussUnit}
                    onChange={(e) => setTrussUnit(e.target.value)}
                    style={{ ...inputStyle, marginLeft: 5 }}
                  >
                    <option>N</option>
                    <option>kN</option>
              ...
              >
                <svg
                  width="1400"
                  height="600"
                  onClick={handleTrussCanvasClick}
                >
                  <defs>
                    <pattern
                      id="trussGrid"
                      width={PIXELS_PER_GRID}
                      height={PIXELS_PER_GRID}
                      patternUnits="userSpaceOnUse"
                    >
                      <path
                        d={`M${PIXELS_PER_GRID} 0 L0 0 0 ${PIXELS_PER_GRID}`}
                        fill="none"
                        stroke="#2a2a2a"
                      />
                    </pattern>
                  ...
                  {elements.map((el) => {
                    const a = nodes.find((n) => n.id === el.n1)
                    const b = nodes.find((n) => n.id === el.n2)
                    return a && b ? (
                      <line
                        key={el.id}
                        x1={a.x}
                  ...
                  {Object.entries(trussLoads).map(([id, f]) => {
                    ...
                    const hasFy = Number(f.fy) !== 0
                    const hasFx = Number(f.fx) !== 0
                    ...
                    return (
                      <g key={id}>
                        {hasFy && (
                          <line
                            x1={n.x}
                            y1={Number(f.fy) > 0 ? n.y - 55 : n.y + 55}
                            x2={n.x}
                            y2={Number(f.fy) > 0 ? n.y - 10 : n.y + 10}
                            stroke={theme.accent}
                            strokeWidth="3"
                            markerEnd="url(#arrowPoint)"
                          />
                        ...
                        {hasFx && (
                          <line
                            x1={Number(f.fx) > 0 ? n.x - 55 : n.x + 55}
                            y1={n.y}
                            x2={Number(f.fx) > 0 ? n.x - 10 : n.x + 10}
                            y2={n.y}
                            stroke={theme.accent}
                            strokeWidth="3"
                            markerEnd="url(#arrowPoint)"
                  ...
                  {nodes.map((n) => (
                    <g key={n.id}>
                      <circle
                        ...
                        r="30"
                        fill="transparent"
                        onClick={(e) => handleTrussNodeClick(e, n)}
                      />
                      <circle
                        cx={n.x}
                        cy={n.y}
                        r={selectedNodeId === n.id ? 9 : 6}
                        fill={selectedNodeId === n.id ? theme.accent : '#222'}
                        stroke="#fff"
                        strokeWidth="2"
                        pointerEvents="none"
                      />
              ...
              >
                {selectedNodeId ? (
                  ...
                  >
                    <b>
                      Node {nodes.find((n) => n.id === selectedNodeId)?.name}
                    </b>
                    <select
                      value={trussSupports[selectedNodeId]?.type || 'none'}
                      onChange={(e) =>
                        handleSupportTypeChange(selectedNodeId, e.target.value)
                      }
                      style={inputStyle}
                    >
                    ...
                    <select
                      value={
                        trussSupports[selectedNodeId]?.direction || 'horizontal'
                      }
                      onChange={(e) => {
                        saveTrussState()
                        setTrussSupports((prev) => ({
                    ...
                    <input
                      placeholder={`Fx (${trussUnit})`}
                      type="number"
                      value={trussLoads[selectedNodeId]?.fx ?? ''}
                      onChange={(e) =>
                        handleTrussLoadChange(
                          selectedNodeId,
                          'fx',
                          e.target.value
                        )
                    ...
                    />
                    <input
                      placeholder={`Fy (${trussUnit}) ↓`}
                      type="number"
                      value={trussLoads[selectedNodeId]?.fy ?? ''}
                      onChange={(e) =>
                        handleTrussLoadChange(
                          selectedNodeId,
                          'fy',
                  ...
                  <span style={{ color: '#888' }}>
                    Click a node to set supports or nodal loads. Positive Fy points downward. Click two nodes to create a member.
                  </span>
                )}
              </div>
              ...
              <div style={{ textAlign: 'center', margin: 18 }}>
                <button
                  style={buttonStyle}
                  disabled={nodes.length < 2 || !elements.length}
                  onClick={runTrussAnalysis}
                >
                  Analyze Truss
                </button>
              </div>
                    ...
                    <tbody>
                      {trussAnalysisResult.members.map((m, i) => (
                        ...
                        >
                          <td>{m.name}</td>
                          <td style={{ textAlign: 'right' }}>
                            {formatNumber(knToForce(m.force, trussUnit))}{' '}
                            {trussUnit}
                          </td>
          ...
          {activeTab === 'frame' && (
            <div className="report-document">
              ...
              >
                <button style={buttonStyle} onClick={loadFramePreset}>
                  Portal Frame
                </button>
                <button
                  style={buttonStyle}
                  onClick={handleUndoFrame}
                  disabled={!frameHistory.length}
                >
                  Undo
                </button>
                <button style={buttonStyle} onClick={clearFrameCanvas}>
                  Clear
                </button>
                ...
                <label>
                  ...
                  <select
                    value={fForceUnit}
                    onChange={(e) => setFForceUnit(e.target.value)}
                    style={{ ...inputStyle, marginLeft: 5 }}
                  >
                    <option>N</option>
                    <option>kN</option>
                ...
                <label>
                  ...
                  <select
                    value={fGridScale}
                    onChange={(e) => setFGridScale(Number(e.target.value))}
                    style={{ ...inputStyle, marginLeft: 5 }}
                  >
                    {Array.from({ length: 30 }, (_, i) => (i + 1) / 10).map((v) => (
                      <option key={v} value={v}>
                ...
                >
                  ...
                  {fElements.map((el) => {
                    ...
                    return (
                      <g
                        key={el.id}
                        onClick={(e) => handleFrameElementClick(e, el)}
                      >
                        <line
                          x1={a.x}
                          y1={a.y}
                          x2={b.x}
                          y2={b.y}
                          stroke={selected ? theme.accent : '#fff'}
                        ...
                        {loc && p?.py && (() => {
                          ...
                          const ny = dx / L
                          const offset = 42
                          const bx = loc.x + nx * offset
                          const by = loc.y + ny * offset
                          const directionY = Number(p.py) > 0 ? 1 : -1
                        ...
                        {loc && p?.px && (() => {
                          ...
                          const bx = loc.x + nx * offset
                          const by = loc.y + ny * offset
                          const directionX = Number(p.px) > 0 ? 1 : -1
                    ...
                    return (
                      <g key={id}>
                        {hasFy && (
                          <line
                            x1={n.x}
                            y1={Number(f.fy) > 0 ? n.y - 50 : n.y + 50}
                            x2={n.x}
                            y2={Number(f.fy) > 0 ? n.y - 10 : n.y + 10}
                            stroke={theme.accent}
                            strokeWidth="3"
                        ...
                        {hasFx && (
                          <line
                            x1={Number(f.fx) > 0 ? n.x - 50 : n.x + 50}
                            y1={n.y}
                            x2={Number(f.fx) > 0 ? n.x - 10 : n.x + 10}
                            y2={n.y}
                            stroke={theme.accent}
                  ...
                  {fNodes.map((n) => (
                    <g key={n.id}>
                      <circle
                        ...
                        r="30"
                        fill="transparent"
                        onClick={(e) => handleFrameNodeClick(e, n)}
                      />
                      <circle
                        cx={n.x}
                        cy={n.y}
                        r={fSelectedNodeId === n.id ? 9 : 6}
                        fill={fSelectedNodeId === n.id ? theme.accent : '#222'}
                        stroke="#fff"
                        strokeWidth="2"
                        pointerEvents="none"
                      />
              ...
              >
                {fSelectedNode ? (
                  ...
                  >
                    ...
                    <select
                      value={fSupports[fSelectedNode.id]?.type || 'none'}
                      onChange={(e) =>
                        handleFSupportTypeChange(fSelectedNode.id, e.target.value)
                      }
                      style={inputStyle}
                    >
                      <option value="none">None</option>
                    ...
                    <select
                      value={
                        fSupports[fSelectedNode.id]?.direction || 'horizontal'
                      }
                      onChange={(e) => {
                        saveFrameState()
                        setFSupports((prev) => ({
                    ...
                    <input
                      type="number"
                      placeholder={`Fx (${fForceUnit})`}
                      value={fLoads[fSelectedNode.id]?.fx ?? ''}
                      onChange={(e) =>
                        handleFLoadChange(fSelectedNode.id, 'fx', e.target.value)
                      }
                      style={{ ...inputStyle, width: 80 }}
                    />
                    <input
                      type="number"
                      placeholder={`Fy (${fForceUnit}) ↓`}
                      value={fLoads[fSelectedNode.id]?.fy ?? ''}
                      onChange={(e) =>
                        handleFLoadChange(fSelectedNode.id, 'fy', e.target.value)
                      }
                      style={{ ...inputStyle, width: 100 }}
                    />
                    <input
                      type="number"
                      placeholder="Mz (kN·m)"
                      value={fLoads[fSelectedNode.id]?.mz ?? ''}
                      onChange={(e) =>
                        handleFLoadChange(fSelectedNode.id, 'mz', e.target.value)
                      }
                  ...
                  >
                    <b>
                      Member{' '}
                      {fNodes.find((n) => n.id === fSelectedElement.n1)?.name}
                      {fNodes.find((n) => n.id === fSelectedElement.n2)?.name}
                    </b>
                    <input
                      type="number"
                      placeholder={`wx (${fForceUnit}/m)`}
                      value={fDistLoads[fSelectedElement.id]?.wx ?? ''}
                      onChange={(e) =>
                        handleFDistLoadChange(fSelectedElement.id, 'wx', e.target.value)
                      }
                      style={{ ...inputStyle, width: 110 }}
                    />
                    <input
                      type="number"
                      placeholder={`wy (${fForceUnit}/m) ↓`}
                      value={fDistLoads[fSelectedElement.id]?.wy ?? ''}
                      onChange={(e) =>
                        handleFDistLoadChange(fSelectedElement.id, 'wy', e.target.value)
                      }
                      style={{ ...inputStyle, width: 120 }}
                    />
                    <input
                      type="number"
                      placeholder={`Px (${fForceUnit})`}
                      value={fPointLoadsOnElement[fSelectedElement.id]?.px ?? ''}
                      onChange={(e) =>
                        handleElementPointLoadChange(fSelectedElement.id, 'px', e.target.value)
                      }
                      style={{ ...inputStyle, width: 90 }}
                    />
                    <input
                      type="number"
                      placeholder={`Py (${fForceUnit}) ↓`}
                      value={fPointLoadsOnElement[fSelectedElement.id]?.py ?? ''}
                      onChange={(e) =>
                        handleElementPointLoadChange(fSelectedElement.id, 'py', e.target.value)
                      }
                      style={{ ...inputStyle, width: 90 }}
                    />
                    <input
                      type="number"
                      placeholder="a from Node i (m)"
                      value={fPointLoadsOnElement[fSelectedElement.id]?.x ?? ''}
                      onChange={(e) =>
                        handleElementPointLoadChange(fSelectedElement.id, 'x', e.target.value)
                      }
                      style={{ ...inputStyle, width: 120 }}
                    />
                  </div>
                  ...
                  <span style={{ color: '#888' }}>
                    Click a node for supports/nodal loads or click a member for distributed/member point loads.
                    <br />
                    Positive Fy is downward.
                  </span>
                )}
              </div>
              ...
              <div style={{ textAlign: 'center', margin: 18 }}>
                <button
                  style={buttonStyle}
                  disabled={fNodes.length < 2}
                  onClick={runFrameStaticsAnalysis}
                >
                  Calculate Frame Reactions
                </button>
              </div>
              ...
              {frameLocalData.analyzed &&
                Object.keys(frameLocalData.rxns).length > 0 && (
                  <div
                    style={{
                      marginTop: 15,
                      padding: 15,
                  ...
                  >
                    ...
                    {Object.entries(frameLocalData.rxns).map(([id, r]) => (
                      <div key={id} style={{ marginBottom: 8 }}>
                        <b>
                          {fNodes.find((n) => String(n.id) === String(id))?.name}
                        </b>
